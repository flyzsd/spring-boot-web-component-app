...
const $ = (shadowRoot: ShadowRoot | null, selectors: any) : HTMLInputElement => {
    return shadowRoot?.querySelector(selectors) as HTMLInputElement;
};
...
class MyCreditCard extends HTMLElement {
    ...
    get _template() {
      ...
      <div id="form-container" class=${classMap({highlight: true, enabled: true, hidden: false})}>
        ...
        <div id="card-front">
          ...
          <button type="button" id="chip" class="chip" @click="${this.handleClick.bind(this)}">
           <svg
    ...
    handleClick(e) {
        ...
        let event = new CustomEvent('credit-card-details', {
            detail: {
                bank: this.title,
                cardNumber: $(this.shadowRoot,'#card-number')?.value,
                cardHolder: $(this.shadowRoot,'#card-holder')?.value,
                expirationMonth: $(this.shadowRoot,'#card-month')?.value,
                expirationYear: $(this.shadowRoot,'#card-year')?.value,
                cvc: $(this.shadowRoot,'#card-cvc')?.value
            },
            composed: true,
            bubbles: true
        });
